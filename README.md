...
- 历史记录：在浏览器本地保留近期占卜记录，方便回看。
- 移动端适配：支持手机端选牌、滑动和阅读体验。

## 数据存储说明

项目不内置账号系统，也不维护云端长期记忆。近期占卜历史保存在当前浏览器的 `localStorage` 中；抽牌后如果 DeepSeek 暂时连接失败，本次牌面会临时保存在当前会话的 `sessionStorage` 中，便于继续使用同一组牌重试解析。

DeepSeek API 调用由服务端 API route 完成。正式解析时只提交当前这次占卜所需的问题、背景、牌阵、牌名、正逆位和牌位含义；不会自动把浏览器历史记录拼入模型上下文。
...
- 更细的本地历史管理，例如筛选、搜索和批量清理。
- 可选的导出功能，例如把单次解读保存为图片或文本。
- 增加更细的牌义语料和不同主题的解析策略。
- 支持解读收藏和删除单条记录。
- 持续优化移动端选牌与结果页阅读体验。

...
按以下四个段落输出，并保留段落标题：
总体结论：直接回答用户问题，说明主要趋势和不确定性。
逐张牌分析：按抽牌顺序逐张分析，每张牌都必须提到牌名、正逆位、牌位含义、基础牌义，以及它对用户问题的对应判断。
综合判断：把牌位关系串联起来，说明整体倾向、矛盾点和需要留意的边界。
行动建议：给出克制、现实、可执行的建议。涉及医疗、法律、投资等高风险问题时，提醒寻求专业人士意见。
